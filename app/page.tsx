...
const presets: Preset[] = [
  { id: "rain", icon: "🌧️", title: "雨停以前", subtitle: "一场被雨困住的安静下午", mood: "温暖", scene: "突如其来的大雨让两个人被困在同一个地方，原本普通的下午因此慢了下来。", required: "雨声、热饮、一个不经意流露出的关心" },
  { id: "store", icon: "🌙", title: "凌晨三点便利店", subtitle: "睡不着的人碰见了另一个睡不着的人", mood: "治愈", scene: "深夜无法入睡时，两个人在便利店或类似的夜间场所相遇，并一起度过一段无人打扰的时间。", required: "夜色、随手买下的小东西、回去路上的对话" },
  { id: "power", icon: "🕯️", title: "停电的那个晚上", subtitle: "灯熄灭以后，距离忽然变近", mood: "暧昧", scene: "住处突然停电，两个人只能借着微弱光线一起处理这场意外。", required: "手电或烛光、一次靠得很近的瞬间、停电恢复后的余韵" },
  { id: "cat", icon: "🐈", title: "一起捡到一只猫", subtitle: "临时的客人搅乱了原本的生活", mood: "甜", scene: "两个人在回家途中遇见一只需要帮助的流浪猫，并决定暂时照顾它。", required: "给猫取名、意见不一致又悄悄妥协、某个像一家人的画面" },
  { id: "kitchen", icon: "🍳", title: "厨房里的小事故", subtitle: "一顿饭差点变成灾难", mood: "搞笑", scene: "两个人一起准备一顿饭，却因为经验、性格或意外状况把厨房弄得一团乱。", required: "失败的食物、互相吐槽、最后仍然一起吃完" },
  { id: "leave", icon: "🧳", title: "没说出口的离开", subtitle: "收拾行李的人被当场发现", mood: "酸涩", scene: "其中一人打算在没有告别的情况下悄悄离开，却在收拾行李时被另一人发现。", required: "被发现的瞬间、压抑已久的情绪、一个明确的选择" },
];

const INTERNAL_PROTOCOL = `【内部演绎协议｜不得向用户展示】
读取本次剧场订单后，先结合当前会话或运行环境中已经加载的世界书、角色卡、人物设定、关系设定、背景资料与近期记忆进行理解，再开始创作。

以世界书确定时代、地点、社会规则、力量体系与客观背景；以角色卡确定人物身份、性格、经历、能力、价值观、语言习惯、行为逻辑与情绪表达方式；以近期记忆确认当前关系阶段、已发生事件、彼此知晓的信息、未解决矛盾及相处细节。

在此基础上演绎用户选择或填写的小剧场，不得让人物脱离原有设定，也不得为套用题材擅自覆盖世界观、改变人物关系或制造与近期经历冲突的情节。若内容与既有设定轻微冲突，应保留用户想看的核心事件并作合理转化；若信息缺失，则结合已有设定自然补全，不展示分析过程。
...
function buildPrompt(values: Omit<Order, "id" | "createdAt" | "prompt">) {
  ...
  return `${INTERNAL_PROTOCOL}\n\n【本次剧场订单】\n剧场标题：${values.title || "请根据情节自然拟定"}\n核心设定：${values.scene || "请结合当前关系与近期记忆，自然生成一个适合人物的小剧场"}\n必须出现：${values.required || "无硬性要求"}\n避免出现：${values.avoid || "无额外限制"}\n情绪氛围：${values.mood}\n篇幅要求：${values.cupSize}｜${length[values.cupSize]}\n叙事偏好：${values.narrative}\n\n请根据世界书、角色卡及近期记忆演绎本次小剧场，直接输出正文。`;
}
...
export default function Home() {
  ...
  const [mood, setMood] = useState<Mood>("温暖");
  ...
  async function copy(text = prompt) {
    try { await navigator.clipboard.writeText(text); setToast("演绎指令已复制，可以粘贴到聊天中"); }
    catch { setToast("复制失败，请长按文本手动复制"); }
  }
    ...
    <main className="cafe-shell">
      ...
      <section className="hero" id="top">
        <div className="hero-copy">
          ...
          <h1>点一杯咖啡，<br />看一段只属于<span>你们</span>的故事。</h1>
          <p className="hero-text">选择今日菜单，或亲手写下一张故事订单。咖啡馆会沿着世界书、角色卡与近期记忆，继续演绎你们已经走过的轨迹。</p>
          <div className="hero-actions"><a className="primary-button" href="#menu">看看今日菜单</a><button className="ghost-button" onClick={randomPreset}>🎲 随机来一杯</button></div>
        </div>
        <div className="hero-card"><span className="steam steam-one">~</span><span className="steam steam-two">~</span><div className="cup-art">☕</div><p>今日推荐</p><h2>雨停以前</h2><span>温暖 · 中杯 · 慢慢展开</span><button onClick={() => loadPreset(presets[0])}>端上这一杯</button></div>
      </section>

      <section className="section" id="menu">
        <div className="section-heading"><div><p className="eyebrow">TODAY&apos;S THEATRE MENU</p><h2>今日小剧场</h2></div><button className="text-button" onClick={randomPreset}>换一杯灵感 ↻</button></div>
        <div className="preset-grid">{presets.map((item) => <button className="preset-card" key={item.id} onClick={() => loadPreset(item)}><span className="preset-icon">{item.icon}</span><span className="preset-mood">{item.mood}</span><strong>{item.title}</strong><small>{item.subtitle}</small><span className="order-link">点这一杯 →</span></button>)}</div>
      </section>

      <section className="section studio-section" id="studio">
        <div className="section-heading"><div><p className="eyebrow">CUSTOM THEATRE STUDIO</p><h2>定制一杯属于你们的故事</h2></div><button className="text-button" onClick={clearForm}>清空订单</button></div>
      ...
      <section className="section" id="shelf">
        ...
        {history.length === 0 ? <div className="empty-shelf"><span>📖</span><h3>书架还是空的</h3><p>制作第一张故事订单后，它会被保存在这台设备上。</p></div> : <div className="history-grid">{history.map((item) => <article className="history-card" key={item.id}><div><span>{item.mood}</span><time>{formatDate(item.createdAt)}</time></div><h3>{item.title}</h3><p>{item.scene || "根据现有设定自由演绎"}</p><footer><small>{item.cupSize}</small><button onClick={() => { setCurrent(item); copy(item.prompt); }}>再次取用</button></footer></article>)}</div>}
      </section>

      <footer className="site-footer"><strong>CRIMSON CAFÉ</strong><p>点一杯咖啡，看一段只属于 {"{{char}}"} 和 {"{{user}}"} 的故事。</p></footer>
